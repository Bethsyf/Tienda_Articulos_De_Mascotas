export const showDataCarrito = (data, container) => {
    ...
    data.forEach((element, index) => {
        ...
        <img style='width:25px' class='mx-3' src="${element.image}" alt="djdjd">
        <h6 class='mx-5 my-3' id='titulo'>${element.name}</h6>
        <h6 class='mx-5 my-3' id='price'>${element.price}</h6>
        <button style='height: 50px;' id=${perros} class='my-1 btn btn-outline-primary ms-5 restar'>-</button>
        <h6 class='mx-3 my-3' id='cantidad'>${element.units}</h6>
        <button style='height: 50px;' id=${perros} class='my-1 btn btn-outline-primary me-5 sumar'>+</button>
